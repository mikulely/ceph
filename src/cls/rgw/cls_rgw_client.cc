...
{
  bufferlist in, out;
  int r = io_ctx.exec(oid, "rgw", "bl_get_head", in, out);
  if (r < 0)
    return r;
...
int cls_rgw_bl_put_head(IoCtx& io_ctx, string& oid, cls_rgw_bl_obj_head& head)
{
  ...
  call.head = head;
  ::encode(call, in);
  int r = io_ctx.exec(oid, "rgw", "bl_put_head", in, out);
  return r;
}
...
int cls_rgw_bl_get_next_entry(IoCtx& io_ctx, string& oid, string& marker, pair<string, int>& entry)
{
  ...
  call.marker = marker;
  ::encode(call, in);
  int r = io_ctx.exec(oid, "rgw", "bl_get_next_entry", in, out);
  if (r < 0)
    return r;
...
int cls_rgw_bl_rm_entry(IoCtx& io_ctx, string& oid, pair<string, int>& entry)
{
  ...
  call.entry = entry;
  ::encode(call, in);
  int r = io_ctx.exec(oid, "rgw", "bl_rm_entry", in, out);
 return r;
}
                    ...
                    map<string, int>& entries)
{
  ...
  int r = io_ctx.exec(oid, "rgw", "bl_list_entries", in, out);
  if (r < 0)
    return r;
