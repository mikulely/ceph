...
struct rgw_usage_log_entry {
    ...
    for (iter = e.usage_map.begin(); iter != e.usage_map.end(); ++iter) {
      if (!categories || !categories->size() || categories->count(iter->first)) {
        if (e.subuser.empty())
          ...
          add(e.subuser, iter->first, iter->second);
      }
    }
  }
  ...
  void sum(rgw_usage_data& usage, map<string, bool>& categories) const {
    ...
    for(auto subuser_iter = subuser_usage_map.begin(); subuser_iter != subuser_usage_map.end(); ++subuser_iter) {
      for (auto category_iter = (*subuser_iter).second.begin();
           category_iter !=  (*subuser_iter).second.end(); ++category_iter) {
  ...
  void set_subuser(const string& name) {
    subuser = name;
  }
};
