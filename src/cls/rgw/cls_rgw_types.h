...
struct rgw_usage_log_entry {
  ...
  void aggregate(const rgw_usage_log_entry& e, map<string, bool> *categories = NULL) {
    ...
    // aggregate suubuser usage
    for(auto subuser_iter : e.subuser_usage_map) {
  ...
  void sum(const string& subuser, rgw_usage_data& usage, map<string, bool>& categories) const {
    ...
    // sum up usage generated by subuser's key-pairs
    for(auto subuser_iter : subuser_usage_map) {
      if (subuser_iter.first == subuser) {
        for (auto category_iter : subuser_iter.second) {
