...
typedef map<string, rgw_usage_data> usage_map_t;
typedef map<string, usage_map_t > subuser_usage_map_t;
...
struct rgw_usage_log_entry {
  ...
  subuser_usage_map_t subuser_usage_map;
  ...
  void aggregate(const rgw_usage_log_entry& e, map<string, bool> *categories = NULL) {
    ...
    for (const auto iter : e.usage_map) {
      if (!categories || !categories->size() || categories->count(iter.first)) {
        add(iter.first, iter.second);
      }
    }

    // aggregate subuser usage.
    for(const auto subuser_iter : e.subuser_usage_map) {
      for (const auto category_iter : subuser_iter.second) {
        if (!categories || !categories->size() || categories->count(category_iter.first)) {
          add(subuser_iter.first, category_iter.first, category_iter.second);
        }
      }
    }
  }
  ...
  void sum(rgw_usage_data& usage, map<string, bool>& categories) const {
    ...
    // sum up usage generated by user's key-pairs.
    for (const auto iter : usage_map) {
  ...
  void sum(const string& subuser, rgw_usage_data& usage, map<string, bool>& categories) const {
    usage = rgw_usage_data();

    // sum up usage generated by subusers' key-pairs.
    const auto subuser_iter = subuser_usage_map.find(subuser);
    if (subuser_iter != subuser_usage_map.end()) {
      for (const auto category_iter : subuser_iter->second) {
        if (!categories.size() || categories.count(category_iter.first)) {
          usage.aggregate(category_iter.second);
        }
      }
    }
  }
  ...
  void add(const string& subuser, const string& category, const rgw_usage_data& data) {
    subuser_usage_map[subuser][category].aggregate(data);
  }
