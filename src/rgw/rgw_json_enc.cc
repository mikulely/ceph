...
void RGWUserInfo::dump(Formatter *f) const
{
  ...
  if (bl_deliver) { /* no need to show it for every user */
    encode_json("bl_deliver", (bool)bl_deliver, f);
  }
...
void RGWUserInfo::decode_json(JSONObj *obj)
{
  ...
  bool deliver = false;
  JSONDecoder::decode_json("bl_deliver", deliver, obj);
  bl_deliver = (__u8)deliver;
...
void RGWZoneParams::dump(Formatter *f) const
{
  ...
  encode_json_plain("bl_deliver_key", bl_deliver_key, f);
...
void RGWZoneParams::decode_json(JSONObj *obj)
{
  ...
  JSONDecoder::decode_json("bl_deliver_key", bl_deliver_key, obj);
