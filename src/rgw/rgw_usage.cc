...
{
  formatter->open_array_section("categories");
  const auto subuser_iter = entry.subuser_usage_map.find(subuser);
		   ...
		   RGWFormatterFlusher& flusher)
{
  ...
  if (!by_subuser)
    RGWUsage::show(store, uid, start_epoch, end_epoch, show_log_entries, show_log_sum, categories, flusher);
  ...
  while (is_truncated) {
    ...
    for (iter = usage.begin(); iter != usage.end(); ++iter) {
      ...
      const auto subiter = entry.subuser_usage_map.find(subuser);
